...
{
    if (onoff == led_state[0].previous)
    {
        ESP_LOGW(TAG, "led %s is already %s",
                 led_state[0].name, (onoff ? "on" : "off"));
        return;
    }
                                               ...
                                               esp_ble_mesh_generic_server_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_GENERIC_SERVER_STATE_CHANGE_EVT:
        ...
        if (param->ctx.recv_op == ESP_BLE_MESH_MODEL_OP_GEN_LEVEL_SET ||
            ...
            ESP_LOGI(TAG, "%d", param->value.state_change.level_set.level);
            long level_maped = 0;
            long level = param->value.state_change.level_set.level;
            level_maped = map(level, -32767, 32767, 0, 100);
            ESP_LOGI(TAG, "%d", level_maped);
        }
        break;
